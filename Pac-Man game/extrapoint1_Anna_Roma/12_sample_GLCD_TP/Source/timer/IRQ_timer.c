...
** Descriptions:        qui gestisco i timer. In questo progetto si utilizzano due timer:
- timer 0: gestisce la velocità del PacMan. Qui è contenuta la logica che determina il movimento del PacMan sulla base 
delle indicazioni inviate tramite joistick
- timer 1: gestisce il countdown da 60 a 0 secondi. Al termine stampa un messaggio di GAME OVER e termina il gioco. inoltre questo
timer serve anche per stampare casualmente le PowerPills, in quanto estrae 6 numeri casuali da 0 a 58 e stampa nel display la Powerpill 
nei 6 secondi estratti casualmente, nelle posizioni casuali estratte nella funzione DrawMaze chiamata nell'inizializzazione allo start del programma
...
extern int posizioniCiboGrande[6][2];
		...
		if (score == 1000 || score == 1500 || score == 2000) {
				...
				switch(contatore) {
					case 1: 
						...
						break;
					case 8: 
						LCD_DrawFilledRectangle(140, 40, 150, 20, White); //pulisco
						PrintNumber(140, 40, score, Purple, White);
						break;
				}
		}
...
void TIMER1_IRQHandler(void) {
    static int countdown = 60; // Variabile statica per mantenere il valore tra chiamate
		...
		if (countdown == 60) {
			estraiPosizioniCasuali(mat, posizioniCiboGrande);
		}
    ...
    if (countdown > 0) {
				...
				for (i = 0; i < 6; i++) {
					if (countdown == numeri[i]) {
						numeri[i]=0;
						int riga = posizioniCiboGrande[conteggioCiboGrande][0];
						int colonna = posizioniCiboGrande[conteggioCiboGrande][1];
						conteggioCiboGrande++;
						// Calcola coordinate display
						int x = colonna * 7 + 20; // Posizione orizzontale
						int y = riga * 7 + 70;    // Posizione verticale

						LCD_DrawFilledCircle(x + 3, y + 3, 3, 0x5268); // Cibo grande Darkgray 
						mat[riga][colonna] = 3; 
					} }
