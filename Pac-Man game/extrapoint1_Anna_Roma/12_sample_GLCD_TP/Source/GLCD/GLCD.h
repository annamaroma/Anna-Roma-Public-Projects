...
#define Purple				 0x9008
#define Darkgray			 0x5268
#define Brown					 0xCC68
...
void TopInitialize(void); //inizializzo l'inizial score e il countdown
void LCD_DrawFilledCircle(uint16_t x0, uint16_t y0, uint16_t radius, uint16_t color); //funzione per disegnare un cerchio pieno
void LCD_DrawFilledRectangle(uint16_t x0, uint16_t y0, uint16_t width, uint16_t height, uint16_t color); //funzione per disegnare un rettangolo pieno
// DrawMaze disegna la matrice contenente il gioco
...
void DownInitialize(uint16_t Vite); //disegno le vite rimanenti
void estraiNumeriCasuali(int numeri[], int n, int max); //estrae n numeri casuali da 0 a max
void estraiPosizioniCasuali(int mat[31][28], int posizioni[6][2]); //estrae posizioni casuali delle Powerpills pescandole tra le posizioni delle Standard Pills
